  ...
  const columns: ColumnDef<User>[] = [
    {
      accessorKey: 'name',
      header: 'Name',
      cell: ({ row }) => h(StudentViewCard, { user: row.original }),
    },
    {
      cell: ({ row }) => {
        const icons = []
      ...
      cell: ({ row }) => {
        return h('div', { class: 'flex gap-2' }, [
          h(ScheduleDrawer, { student: row.original }, () => [
            h(IconButton, { hintText: `View schedule` }, () => [h(CalendarDays)]),
          ]),
          h(
            StudentDialog,
            { isEdit: true, student: row.original, refresh: () => getStudents() },
            () => [
          ...
          h(
            ConfirmDialog,
            {
              ...
              actionText: 'Remove',
              action: () => deleteStudent(row.original),
              refresh: () => getStudents(),
            },
            () => [
