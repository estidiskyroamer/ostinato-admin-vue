...
export const addGrade = async ({ name, tuitionFee }: {
    name: string;
    tuitionFee: Number;
}): Promise<Grade | null> => {
    try {
        const response = await axiosInstance.post('/admin/grades',
            { 'name': name, 'tuitionFee': tuitionFee });
...
export const updateGrade = async ({ gradeId, name, tuitionFee }: {
    gradeId: string;
    name: string;
    tuitionFee: string;
}): Promise<Grade | null> => {
...
export const deleteGrade = async (gradeData: Grade): Promise<Grade | null> => {
    try {
        const response = await axiosInstance.delete<ApiResponse<Grade>>('/admin/grades/grade/' + gradeData.id);
        return response.data.data;
    } catch (error) {
        console.log(error);
        return null;
...
export const getGradeListPaginated = async (page: number): Promise<Paginated<Grade[]> | null> => {
    try {
        const response = await axiosInstance.get<ApiResponse<Paginated<Grade[]>>>('/admin/grades?limit=25&page=' + page);
        return response.data.data;
    } catch (error) {
        return null;
        /* if (error instanceof AxiosError) {
